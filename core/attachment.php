...
{
	if (strpos($refer_url,SYS_HTTP.SYS_HOST)!=0) message('附件禁止从地址栏直接输入或从其他站点链接访问', './');
}
 ...
 * @文件下载
*/
if ($host['attach_display'] && ($sendkey!=$downkey || $refer_url != $query_url) && !$attachinfo['isimage'])//显示下载页面后再下载,//
{
	$title = $attachinfo['filename'].'-文件下载';
	setcookie($downid,$downkey,$timestamp+3600);
}
else
{
	$filepath = SYS_DATA.'/files/'.$attachinfo['filepath'];
