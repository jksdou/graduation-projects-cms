...
if ($searchkey)
{
	...
	//写入搜索数据库日志
	$DB->query('INSERT INTO `'.DB_PREFIX."search` (`hostid`, `user`, `dateline`, `ip`, `useragent`, `keywords`) VALUES ('$hostid', '$username', '$timestamp', '$onlineip', '$useragent', '$searchkey')"); 
	//写入搜索文件日志
	writeLog(SYS_DATA.'/logs/search.log', $hostid."\t".$username."\t".$timestamp."\t".$onlineip."\t".$useragent."\t".$searchkey);
	...
	$query_sql = 'SELECT * FROM `'.DB_PREFIX."article` WHERE `visible` = '1' AND `hostid` = '$hostid' AND ($sqltxtsrch) ORDER BY $sortby $orderby LIMIT $start,{$host['list_shownum']}";
