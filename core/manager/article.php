<?php
include SYS_CORE.'/include/article.php';
		...
		case 'mod'://修改文章
			...
			if (!empty($keepattach)&&is_array($keepattach)&&count($keepattach)<count($oldattachids))
			{
				...
				if ($diffids) 
				{
					...
					{
						$filepath=SYS_DATA.'/files/'.$dfetch['filepath'];
						if (file_exists($filepath)) @unlink($filepath);
						$thumbpath=SYS_DATA.'/files/'.$dfetch['thumb_filepath'];
...
else
{
	...
	else if ($action=='list')
	{
		...
		if (empty($tag))
		{
			...
			$query = $DB->query('SELECT a.*,c.name AS cname FROM `'.DB_PREFIX."article` a LEFT JOIN `".DB_PREFIX."category` c ON c.cid=a.cateid WHERE a.hostid = '$hostid' $searchsql $addquery $uquery ORDER BY a.aid DESC LIMIT $start_limit, 30");
		}
		else
		{
			$item = addslashes($tag);
