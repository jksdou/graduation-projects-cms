<?php
include SYS_CORE.'/include/tag.php';
...
if (SYS_POST)
{
	if ($action=='modredirect'||$action=='addredirect')
	{
		...
		if (!$old)  redirect('原网址不得为空', 'admin.php?file=optimize&action=addredirect');
		if (!$new)  redirect('转向地址不得为空', 'admin.php?file=optimize&action=addredirect');
	}
	switch($action)
	{
		case 'modtag':
			...
			redirect('修改Tags成功', 'admin.php?file=optimize&action=taglist');
			break;
		case 'deltag':
			//批量删除Tag
			if (!isset($_POST['tag']))
			{
				redirect('未选择任何Tags','admin.php?file=optimize&action=taglist');
			}
			...
			redirect('成功删除所选Tags', 'admin.php?file=optimize&action=taglist');
			break;
		case 'delredirect':
			//批量删除Tag
			if (!isset($_POST['rid']))
			{
				redirect('未选择任何跳转网址','admin.php?file=optimize&action=taglist');
			}
			$vids=implode_ids($_POST['rid']);
			$DB->query('delete from '.DB_PREFIX."redirect where hostid=$hostid and rid in ($vids)");
			redirect_recache();
			redirect('成功删除所选跳转网址', 'admin.php?file=optimize&action=redirect');
			break;
		case 'modredirect':
			//修改
			if (!isset($_POST['rid']))
			{
				redirect('未指定修改的网址','admin.php?file=optimize&action=redirect');
			}
			$rid=intval($_POST['rid']);
			$search=$DB->fetch_first('select * from '.DB_PREFIX."redirect where old='$old' and rid!=$rid and hostid=$hostid");
			if ($search) redirect('原地址不能同时转向多个地址', 'admin.php?file=optimize&action=addredirect');
			$DB->query("update ".DB_PREFIX."redirect set `old`='$old',`new`='$new',`status`='$status' where rid=$rid and hostid=$hostid");
			redirect_recache();
			redirect('成功修改网址跳转','admin.php?file=optimize&action=redirect');
			break;
		case 'addredirect';
			$search=$DB->fetch_first('select * from '.DB_PREFIX."redirect where old='$old' and hostid=$hostid");
			if ($search) redirect('该原转向地址已经存在了，请检查', 'admin.php?file=optimize&action=addredirect');
			$DB->query('insert into `'.DB_PREFIX."redirect` (`hostid`,`old`,`new`,`status`) values ('$hostid','$old','$new','$status')");
			redirect_recache();
			redirect('成功添加网址跳转','admin.php?file=optimize&action=redirect');
		default:
			redirect('未定义操作', 'admin.php?file=optimize');
	}
}
...
if ($action=='taglist')
{
	...
	$total = $rs['tags'];
	$multipage = multi($total, 30, $page, 'admin.php?file=optimize&action=taglist');
...
else if ($action == 'modredirect')
{
	...
	if (!$redirectdb) redirect('不存在的转向网址记录', 'admin.php?file=optimize');
