<?php
if (!$action) $action = 'dbinfo';
$backupdir = SYS_DATA.'/backup';
...
function checkSqlFileInfo($sqlfile)
{
	...
	if ($fp)
	{
		...
		if (!empty($dumpinfo))
		{
			if (preg_match('/#'. SYS_NAME .' VERSION: '. SYS_VERSION .'/', $dumpinfo[0]) === 0)
			{
				redirect('导入失败！此文件只能导入到系统版本为' . SYS_VERSION . '的站点!');
			}
			if (preg_match('/#PREFIX:'. DB_PREFIX .'/', $dumpinfo[2]) === 0)
			{
				redirect('导入失败！文件中的数据库前缀与当前系统数据库前缀不匹配' . $dumpinfo[2]);
			}
		}
		else
		{
			redirect('导入失败！此文件不是本系统备份文件!');
		}
	}
	else
	{
		redirect('导入失败！文件读取失败');
	}
}
...
if ($action == 'dobackup')
{
	$sqlfilename = SYS_DATA.'/backup/'.$_POST['filename'].'.sql';
	...
	if ($bakplace == 'local')
	{
		...
		header('Content-Type: text/x-sql');
		header('Expires: '. gmdate('D, d M Y H:i:s', $timestamp) . ' GMT');
		header('Content-Disposition: attachment; filename=SYS_'. gmdate('Ymd', $timestamp+$host['server_timezone']*3600).'.sql');
		if (preg_match("/MSIE ([0-9].[0-9]{1,2})/", $_SERVER['HTTP_USER_AGENT']))
		{
			header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
			header('Pragma: public');
	...
	else
	{
		...
		if ($fp)
		{
			...
			{
				@fclose($fp);
				redirect('备份失败。备份目录('.SYS_DATA.'/backup)不可写');
			}
			else
			{
		...
		{
			redirect('创建备份文件失败。备份目录('.SYS_DATA.'/backup)不可写');
		}
	}
	}
	else
...
if ($action == 'dbinfo')
{
	...
	$system_table_num = $system_table_rows = $system_data_size = $system_index_size = $system_free_size = 0;
	$other_table_num = $other_table_rows = $other_data_size = $other_index_size = $other_free_size = 0;
	$system_table = $other_table = array();
	while($table = $DB->fetch_array($query))
	{
		if (in_array($table['Name'],$tables))
		{
			$system_data_size = $system_data_size + $table['Data_length'];
			$system_index_size = $system_index_size + $table['Index_length'];
			$system_table_rows = $system_table_rows + $table['Rows'];
			$system_free_size = $system_free_size + $table['Data_free'];
	...
	$system_data_size = sizecount($system_data_size);
	$system_index_size = sizecount($system_index_size);
	$system_free_size = sizecount($system_free_size);
