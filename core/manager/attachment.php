...
if (SYS_POST)
{
	switch($action)
	{
		...
		case 'dorepair':
			...
			while ($attach = $DB->fetch_array($query))
			{
				if (!file_exists(SYS_DATA.'/files/'.$attach['filepath']))
				{
		...
		case 'addattachtoarticle':	//添加附件到指定文章
			...
			include_once SYS_CORE.'/include/article.php';
...
else
{
	...
	if ($action == 'repair') $navtitle = '附件修复';
	if ($action == 'clear') $navtitle = '附件清理';
	if ($action == 'list') $navtitle = '附件管理';
}